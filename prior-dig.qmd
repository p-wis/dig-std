...
predyktory <- c("age", "sex", "race", "ejf_per", "chestx", "bmi", "nyha_class",
             "diuret", "aceinhib", "prevmi", "diabetes", "hyperten")

df_trtmt_ <- df_trtmt %>%
  drop_na(all_of(predyktory))
...
df_matched %>%
  ...
  add_difference() %>%
  modify_header(label = "**Zmienna**") %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Po dopasowaniu**")
```
