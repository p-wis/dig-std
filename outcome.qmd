...
```{r}
m1 <- cph(dwhf_surv ~ age + sex + race + ejf_per + chestx  +  bmi + nyha_class +
...
# Model wieloczynnikowy ulepszony, po selekcji kandydatów

```{r}
m2 <- cph(dwhf_surv ~ age + sex + race + rcs(ejf_per, 4) + rcs(chestx, 4)  +  rcs(bmi, 4) + nyha_class +
            diguse + diuret + aceinhib + prevmi + diabetes + hyperten,
          data = df_trtmt, x = TRUE, y = TRUE, surv = TRUE)
