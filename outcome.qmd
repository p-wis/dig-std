...
# jednoczynnikowa ocena potencjalnych predyktorów

W tabeli wyniki regresji 1 czynnikowych m. Coxa dla zm. zdarzenia dwhf oraz 
wybranych predyktorów  

```{r}
vars <- c("age", "sex", "race", "ejf_per", "bmi", "creat", "klevel", "chfdur", 
          "heartrte", "diabp", "sysbp", "functcls", "chfetiol", "prevmi", "angina",
          "diabetes", "hyperten", "diguse", "diuretk", "diuret", "ksupp", "aceinhib",
          "nitrates", "hydral", "vasod", "digdose")

categorical_vars <- c("sex", "race", "functcls", "chfetiol")

dwhf_cox_1v <- function(var_name, categorical_vars = NULL) {
  
  # Pull out the actual variable from the data
  var_data <- df_trtmt[[var_name]]

  # Decide if variable is categorical
  is_categorical <- var_name %in% categorical_vars ||
                    is.factor(var_data) ||
                    is.character(var_data)

  # If categorical, convert to factor
  if (is_categorical) {
    df_trtmt[[var_name]] <- factor(var_data)
  }
...
results <- do.call(rbind, lapply(vars, dwhf_cox_1v, categorical_vars)) %>%
